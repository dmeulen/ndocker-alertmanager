...
RUN \
  ...
  curl -L https://github.com/prometheus/alertmanager/releases/download/v${ALERTMANAGER_VERSION}/alertmanager-${ALERTMANAGER_VERSION}.linux-amd64.tar.gz -o /tmp/alertmanager.tgz && \
  tar xzf /tmp/alertmanager.tgz -C /tmp && \
  mv /tmp/alertmanager-${ALERTMANAGER_VERSION}.linux-amd64/alertmanager /usr/bin && \
